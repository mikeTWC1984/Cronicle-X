Class.subclass(Page.Base, "Page.Schedule", {
	...
	run_event_from_edit: function (e) {
		...
		if ($("#fe_ee_debug_webhook").is(":checked")) { event.web_hook = ""; }
		if ($("#fe_ee_debug_history").is(":checked")) { event.params._overrideJobAction = "job_complete_debug"; }
	...
	get_event_edit_html: function () {
		...
		if (event.id) {
			html += get_form_table_row('Debug', `
				  ...
				  `);
			html += get_form_table_caption("Temporarily override 'Run Now' behavior for debugging purposes");
			html += get_form_table_spacer();
		} //
